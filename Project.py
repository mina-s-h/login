...
###########################################################################################################  Project
    def userbuy(self):
        ...
        sql = '''SELECT products.pname, transactions.qnt, products.bprice FROM products INNER JOIN transactions
    ...
    def delete(self):
        ...
        if len(rows) == 0:
            print("it was wrong")
            return
        sql2 = '''DELETE FROM users WHERE username=?'''
        cnt.execute(sql2, (user,))
        cnt.commit()
        print("your deleted successfully")
